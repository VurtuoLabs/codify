...
Codify is a service employee agent that turns a technician's resolution recap into a knowledge article, a tagged root cause, and a sweep of related open cases.
...
| Classes declared `with sharing`                 | 12    |
| Classes declared `without sharing`              | 0     |
| Production SOQL queries                         | 30    |
| Queries carrying an explicit enforcement clause | 30    |
...
**Writes are gated on explicit human confirmation.** Article drafts and case field changes are proposed to the user and written only after explicit confirmation in the conversation.
...
- It does not publish knowledge articles. It drafts them for a human to review and publish.
- It does not close cases on its own.
- It does not call any external service. There are no outbound callouts and no Named Credentials.
